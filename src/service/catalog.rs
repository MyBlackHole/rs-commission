...
pub async fn active_account(
    ...
    if !account.active || account.kind != kind {
        return Err(Error::invalid(format!("需要启用的 {kind} 账户")));
    }
    Ok(account)
}
...
pub async fn create_account(
    ...
    if !["merchant", "promoter"].contains(&input.kind.as_str())
        || (input.parent_id.is_some() && input.kind != "promoter")
    {
        return Err(Error::invalid(
            "只允许创建商家或推广员账户，只有推广员可设置上级",
        ));
    }
    if input.external_id.starts_with("__") {
        return Err(Error::invalid("__ 前缀保留给系统账户"));
    }
    if let Some(parent) = input.parent_id {
        active_account(&mut work.tx, parent, "promoter").await?;
    }
    let id = Uuid::new_v4();
    ...
    sqlx::query("INSERT INTO wallets(account_id) VALUES($1)")
        ...
        .execute(&mut *work.tx)
        .await?;
    work.complete(serde_json::to_value(account)?, Some(id))
        .await
}
...
pub async fn bind_referral(
    ...
    sqlx::query("SELECT pg_advisory_xact_lock(hashtextextended($1, 9151))")
        .bind(&input.customer_external_id)
        .execute(&mut *work.tx)
        .await?;
    let ordered: bool =
        sqlx::query_scalar("SELECT EXISTS(SELECT 1 FROM orders WHERE customer_external_id=$1)")
            .bind(&input.customer_external_id)
            .fetch_one(&mut *work.tx)
            .await?;
    if ordered {
        return Err(Error::conflict("客户已有入账订单，不能补绑或改变推广关系"));
    }
    sqlx::query("INSERT INTO referral_bindings(customer_external_id,promoter_id) VALUES($1,$2)")
        .bind(&input.customer_external_id)
        .bind(input.promoter_id)
        .execute(&mut *work.tx)
        .await?;
...
pub async fn create_rule(
    ...
    if input.min_base_minor.0 < 0
        || input.min_base_minor.0 > crate::money::MAX_OPERATION_MINOR
        || input
            .max_base_minor
            .is_some_and(|m| m <= input.min_base_minor || m.0 > crate::money::MAX_OPERATION_MINOR)
        || !(-10_000..=10_000).contains(&input.priority)
    {
        return Err(Error::invalid("规则基数范围或优先级无效"));
    }
    if let Some(merchant) = input.merchant_id {
        active_account(&mut work.tx, merchant, "merchant").await?;
    }
    let effective = input.effective_from.unwrap_or_else(Utc::now);
    if input
        .effective_until
        .is_some_and(|until| until <= effective)
    {
        return Err(Error::invalid("失效时间必须晚于生效时间"));
    }
    let id = Uuid::new_v4();
    let t = &input.terms;
...
pub async fn quote_in(tx: &mut Transaction<'_, Postgres>, input: &QuoteInput) -> Result<Quoted> {
    ...
    .bind(input.commission_base_minor)
    .fetch_optional(&mut **tx)
    .await?
...
pub async fn quote(pool: &PgPool, actor: &Actor, input: QuoteInput) -> Result<Value> {
    ...
    tx.rollback().await?;
    Ok(
        json!({"rule":quote.rule,"direct_account_id":quote.direct,"indirect_account_id":quote.indirect,"split":quote.split,"binding":false}),
    )
}
...
pub async fn revoke_credential(pool: &PgPool, actor: &Actor, key: &str, id: Uuid) -> Result<Value> {
    actor.require(&[])?;
    let mut work =
        match Work::begin(pool, actor, "credentials.revoke", key, &json!({"id":id})).await? {
            Start::Replay(v) => return Ok(v),
            Start::New(w) => w,
        };
    sqlx::query("SELECT pg_advisory_xact_lock(734190821)")
        .execute(&mut *work.tx)
        .await?;
    ...
    if role == "admin" && valid {
        let count: i64 = sqlx::query_scalar("SELECT count(*) FROM credentials WHERE role='admin' AND revoked_at IS NULL AND expires_at>now()")
            .fetch_one(&mut *work.tx).await?;
        if count <= 1 {
            return Err(Error::conflict("不能撤销最后一个有效管理员令牌"));
        }
    }
    sqlx::query("UPDATE credentials SET revoked_at=COALESCE(revoked_at,now()) WHERE id=$1")
        ...
        .execute(&mut *work.tx)
        .await?;
    work.complete(json!({"id":id,"revoked":true}), Some(id))
        .await
}
