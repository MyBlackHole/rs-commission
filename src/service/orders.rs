...
pub async fn capture(
    ...
    if input.currency != "CNY" {
        return Err(Error::invalid("当前版本仅支持 CNY，金额单位为分"));
    }
    ...
    if let Some(direct) = quoted.direct {
        parts.push((0, direct, "direct", quoted.split.direct_minor));
    }
    if let Some(indirect) = quoted.indirect {
        parts.push((1, indirect, "indirect", quoted.split.indirect_minor));
    }
    ...
    ledger::post(
        &mut work.tx,
        Posting {
            event_key: format!("capture:{id}"),
            kind: "capture",
            order_id: Some(id),
            payout_id: None,
            actor_id: Some(actor.id),
            metadata: json!({"external_id":input.external_id}),
            lines,
        },
    )
    .await?;
    work.complete(
        json!({"order":order,"allocations":allocations,"split":quoted.split}),
        Some(id),
    )
    .await
}
...
pub async fn refund(
    ...
    let mut work = match Work::begin(
        ...
        "orders.refund",
        key,
        &json!({"order_id":order_id,"input":input}),
    )
    .await?
    {
        Start::Replay(v) => return Ok(v),
    ...
    let cumulative = order
        .refunded_minor
        .0
        .checked_add(input.amount_minor.0)
        .ok_or_else(|| Error::invalid("金额溢出"))?;
    if cumulative > order.paid_minor.0 {
        return Err(Error::conflict("累计退款不能超过订单实付金额"));
    }
    ...
    let bucket = if order.released_at.is_some() {
        Bucket::Available
    } else {
        Bucket::Frozen
    };
    ...
    ledger::post(
        &mut work.tx,
        Posting {
            event_key: format!("refund:{refund_id}"),
            kind: "refund",
            order_id: Some(order_id),
            payout_id: None,
            actor_id: Some(actor.id),
            metadata: json!({"refund_id":refund_id,"external_id":input.external_id}),
            lines,
        },
    )
    .await?;
    let updated =
        sqlx::query_as::<_, Order>("UPDATE orders SET refunded_minor=$2 WHERE id=$1 RETURNING *")
            .bind(order_id)
            .bind(cumulative)
...
async fn release_locked(
    ...
    if order.released_at.is_some() {
        return Ok(json!({"order":order,"changed":false}));
    }
    let due: bool = sqlx::query_scalar("SELECT now() >= $1::TIMESTAMPTZ")
        .bind(order.unlock_at)
        .fetch_one(&mut **tx)
        .await?;
    if !due {
        return Err(Error::conflict("冻结期尚未结束，不能提前解冻"));
    }
    ...
    ledger::post(
        tx,
        Posting {
            event_key: format!("release:{}", order.id),
            kind: "release",
    ...
    .await?;
    let updated =
        sqlx::query_as::<_, Order>("UPDATE orders SET released_at=now() WHERE id=$1 RETURNING *")
            .bind(order.id)
            .fetch_one(&mut **tx)
            .await?;
