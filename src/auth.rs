use crate::{
    error::{Error, Result},
    model::{text, Actor, CreateCredential},
    transaction::{digest, record_event},
    AppState,
};
use axum::{
...
pub fn validate_secret(secret: &str) -> Result<()> {
    let rest = secret
        .strip_prefix("cms_")
        .ok_or_else(|| Error::invalid("令牌格式为 cms_ 加 64 位小写十六进制随机字符"))?;
    if rest.len() != 64
        || !rest
            .bytes()
...
pub async fn insert_credential(
    ...
    if let Some(account) = input.account_id {
        ...
        if !eligible {
            return Err(Error::invalid("成员令牌必须关联启用的商家或推广员"));
        }
    }
    let id = Uuid::new_v4();
    ...
    Ok(
        json!({"id":id, "name":input.name, "role":input.role, "account_id":input.account_id, "expires_at":expires}),
    )
}
...
pub async fn bootstrap(pool: &PgPool) -> Result<Value> {
    ...
    sqlx::query("SELECT pg_advisory_xact_lock(734190821)")
        .execute(&mut *tx)
        .await?;
    let existing: i64 = sqlx::query_scalar("SELECT count(*) FROM credentials")
        .fetch_one(&mut *tx)
        .await?;
    if existing != 0 {
        return Err(Error::conflict(
            "已有凭据，拒绝重复初始化。请通过现有管理员轮换令牌",
        ));
    }
