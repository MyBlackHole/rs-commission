...
impl Work {
    pub async fn begin<T: Serialize + Sync>(
        pool: &PgPool,
        actor: &Actor,
        operation: &str,
        key: &str,
        ...
        if !(8..=128).contains(&key.len()) || !key.bytes().all(|c| c.is_ascii_graphic()) {
            return Err(Error::invalid(
                "Idempotency-Key 必须为 8..128 个可见 ASCII 字符",
            ));
        }
        ...
        if stored_hash != hash {
            return Err(Error::conflict("同一幂等键不能用于不同请求"));
        }
...
pub async fn record_event(
    ...
    sqlx::query(
        "INSERT INTO audit_events(actor_id, action, target_id, detail) VALUES($1,$2,$3,$4)",
    )
    .bind(actor)
    .bind(action)
    .bind(target)
