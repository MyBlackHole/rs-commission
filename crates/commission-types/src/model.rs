...
impl Actor {
    pub fn require(&self, roles: &[&str]) -> Result<()> {
        if self.role == "admin" || roles.contains(&self.role.as_str()) {
            Ok(())
        } else {
            Err(Error::Forbidden)
        }
    ...
    pub fn check_account(&self, id: Uuid) -> Result<()> {
        if self.role == "member" && self.account_id != Some(id) {
            Err(Error::NotFound)
        } else {
            Ok(())
        }
...
pub struct Wallet {
    pub account_id: Uuid,
    pub frozen_minor: Money,
    pub available_minor: Money,
    pub reserved_minor: Money,
...
pub struct Rule {
    ...
    pub priority: i32,
    pub min_base_minor: Money,
    pub max_base_minor: Option<Money>,
    pub rate_bps: i32,
...
impl Rule {
    pub fn terms(&self) -> Terms {
        Terms {
            ...
            fixed_minor: self.fixed_minor,
            cap_minor: self.cap_minor,
            direct_bps: self.direct_bps,
            indirect_bps: self.indirect_bps,
...
pub struct Order {
    ...
    pub currency: String,
    pub paid_minor: Money,
    pub commission_base_minor: Money,
    pub fee_pool_minor: Money,
    pub refunded_minor: Money,
    pub rule_id: Uuid,
...
impl CaptureOrder {
    pub fn quote_input(&self) -> QuoteInput {
        QuoteInput {
            ...
            customer_external_id: self.customer_external_id.clone(),
            paid_minor: self.paid_minor,
            commission_base_minor: self.commission_base_minor,
        }
    }
}
#[derive(Debug, Clone, Serialize, Deserialize)]
