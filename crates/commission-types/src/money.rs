...
impl Money {
    ...
    pub fn from_yuan(value: &str) -> Result<Self, &'static str> {
        ...
        let fraction = match fraction.len() {
            0 => 0, 1 => fraction.parse::<i128>().map_err(|_| "金额格式无效")? * 10,
            _ => fraction.parse::<i128>().map_err(|_| "金额格式无效")?,
        };
        let cents = whole.checked_mul(100).and_then(|v| v.checked_add(fraction)).ok_or("金额超出范围")?;
        i64::try_from(if negative { -cents } else { cents }).map(Self).map_err(|_| "金额超出范围")
    }
}
#[cfg(test)]
mod tests {
    ...
    fn decimal_roundtrip_including_boundaries() {
        for value in [i64::MIN, -101, -1, 0, 1, 12345, 9_007_199_254_740_993, i64::MAX] {
    ...
    fn rejects_rounding_overflow_and_ambiguous_input() {
        for value in ["", "1.234", "1.", ".1", "+1", " 1", "1e2", "NaN", "--1", "92233720368547758.08"] {
