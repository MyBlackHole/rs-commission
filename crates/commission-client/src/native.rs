...
impl State {
    fn session(&self, id: Uuid) -> Result<&Session> {
        self.session.as_ref().filter(|s| s.id == id)
    ...
    fn can_replace(&self) -> bool {
        !self.session.as_ref().and_then(|s| s.pending.as_ref())
            .is_some_and(|p| p.phase.unresolved())
    }
}
impl NativeBridge {
    fn lock(&self) -> Result<MutexGuard<'_, State>> {
        self.0.lock().map_err(|_| invalid("客户端状态异常，请先核验原业务请求"))
    }
    pub async fn login(&self, origin: &str, token: &str) -> Result<NativeSessionInfo> {
        ...
        let generation = Uuid::new_v4();
        {
            ...
            if !state.can_replace() { return Err(invalid("存在未确认写请求，不能更换会话")); }
            state.generation = generation;
            state.session = None;
        }
        let actor = client.me().await?;
        let mut state = self.lock()?;
        if state.generation != generation { return Err(invalid("登录响应已过期")); }
        let info = NativeSessionInfo { id: generation, actor: actor.clone() };
        state.session = Some(Session { id: generation, client, actor, pending: None });
        Ok(info)
    }
    pub fn logout(&self, id: Uuid) -> Result<()> {
        let mut state = self.lock()?;
        state.session(id)?;
        if !state.can_replace() { return Err(invalid("请先核验未确认写请求，不能丢弃会话")); }
    ...
    pub async fn resource(&self, id: Uuid, resource: &str, offset: u32) -> Result<Value> {
        if offset > 1_000_000 { return Err(invalid("分页偏移过大")); }
    ...
    pub fn prepare(&self, id: Uuid, operation: Operation, target: Option<Uuid>, body: &str) -> Result<WriteReceipt> {
        let mut state = self.lock()?;
        let session = state.session_mut(id)?;
        if session.pending.is_some() { return Err(invalid("已有待处理请求，不能覆盖")); }
        if !operation.allowed(&session.actor) { return Err(invalid("当前身份不能执行此操作")); }
        let write = session.client.prepare(operation, target, body)?;
        let receipt = WriteReceipt { id: Uuid::new_v4(), key: write.key().to_owned(), path: write.path().to_owned() };
        session.pending = Some(Pending { receipt: receipt.clone(), write, phase: WritePhase::Prepared, result: None });
        Ok(receipt)
    }
    pub async fn execute(&self, id: Uuid, request: Uuid) -> Result<Value> {
        let (client, write) = {
            let mut state = self.lock()?;
            let session = state.session_mut(id)?;
            let pending = session.pending.as_mut().filter(|p| p.receipt.id == request)
                .ok_or_else(|| invalid("待确认请求不存在或不属于当前会话"))?;
            if let Some(result) = &pending.result { return result.clone(); }
            if !pending.phase.may_send() { return Err(ClientError::Unknown); }
        ...
        if !pending.phase.unresolved() { pending.result = Some(result.clone()); }
        result
    }
    pub fn discard(&self, id: Uuid, request: Uuid) -> Result<()> {
        let mut state = self.lock()?;
        let session = state.session_mut(id)?;
        let pending = session.pending.as_ref().filter(|p| p.receipt.id == request)
            .ok_or_else(|| invalid("待确认请求不存在"))?;
        if !pending.phase.may_edit() { return Err(invalid("结果未知或执行中的请求不能丢弃")); }
