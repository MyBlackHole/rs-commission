...
async fn server(responses: Vec<(&'static str, &'static str)>) -> (String, tokio::task::JoinHandle<Vec<String>>) {
    ...
    let task = tokio::spawn(async move {
        ...
        for (status, body) in responses {
            ...
            let mut request = String::new(); let mut length = 0;
            loop {
                let mut line = String::new();
                assert!(stream.read_line(&mut line).await.unwrap() > 0);
                if let Some(n) = line.to_lowercase().strip_prefix("content-length:") { length = n.trim().parse::<usize>().unwrap(); }
                request.push_str(&line);
                if line == "\r\n" { break; }
                assert!(request.len() < 16_384);
            }
            assert!(length < 65_536);
            let mut input = vec![0; length]; stream.read_exact(&mut input).await.unwrap();
            request.push_str(std::str::from_utf8(&input).unwrap()); seen.push(request);
...
async fn native_bridge_keeps_tokens_and_retry_material_out_of_webview() {
    ...
    let (address, seen) = server(vec![
        ("200 OK", actor),
        ("503 Service Unavailable", r#"{"error":{"code":"busy","message":"retry"}}"#),
        ("200 OK", r#"{"done":true}"#),
    ]).await;
    ...
    assert!(host.prepare(Uuid::new_v4(), Operation::CreateAccount, None, "{}").is_err());
    let write = host.prepare(session.id, Operation::Release, Some(Uuid::nil()), "{}").unwrap();
    assert!(host.prepare(session.id, Operation::Release, Some(Uuid::nil()), "{}").is_err());
    assert!(host.execute(session.id, write.id).await.unwrap_err().outcome_unknown());
    assert!(host.discard(session.id, write.id).is_err());
    assert!(host.logout(session.id).is_err());
    assert_eq!(host.execute(session.id, write.id).await.unwrap()["done"], true);
    // A lost IPC result must replay the cached outcome, not issue a third transfer.
    assert_eq!(host.execute(session.id, write.id).await.unwrap()["done"], true);
    let requests = seen.await.unwrap();
    assert_eq!(requests.len(), 3); assert_eq!(requests[1], requests[2]);
    assert!(requests[1].to_lowercase().contains(&format!("idempotency-key: {}", write.key)));
    let receipt = serde_json::to_string(&write).unwrap();
    assert!(!receipt.contains("native-secret") && !receipt.contains("body"));
    host.discard(session.id, write.id).unwrap(); host.logout(session.id).unwrap();
...
async fn native_bridge_checks_role_and_resource_allowlist() {
    ...
    assert!(host.resource(session.id, "accounts", 1_000_001).await.is_err());
