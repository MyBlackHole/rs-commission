...
(() => {
 ...
 window.__TAURI__={core:{invoke:async(command,args)=>{
   ...
   if(command==='read_resource') return {items:[],has_more:false,offset:0,limit:50};
...
try:
    with sync_playwright() as p:
        ...
        assert set(methods) == {"session_login", "session_logout", "read_resource", "quote_commission", "prepare_write", "execute_write", "discard_write"}, methods
